...
export default function Footer() {
  ...
  const servicesList = content.services && Object.keys(content.services).length > 0
    ? Object.values(content.services).slice(0, 8)
    : [
        { slug: 'amazon-seller-account-management-services', title: 'Amazon Account Management' },
        { slug: 'flipkart-account-management-services', title: 'Flipkart Account Management' },
        { slug: 'blinkit-seller-account-management-services', title: 'Blinkit Quick Commerce' },
        { slug: 'meesho-account-management-services', title: 'Meesho Account Management' },
        { slug: 'shopify-store-management-services', title: 'Shopify Store Management' },
        { slug: 'myntra-account-management-services', title: 'Myntra Account Management' },
        { slug: 'amazon-product-listing-catalogue-services', title: 'Amazon Product Listing & A+' },
        { slug: 'ecommerce-product-listing-services', title: 'Ecommerce Catalog Management' },
      ];

  // Dynamic case studies list
  const caseStudiesList = content.caseStudies && Object.keys(content.caseStudies).length > 0
    ? Object.values(content.caseStudies).slice(0, 4)
    : [
        { slug: 'kay-kay-industries-amazon-case-study', brandName: 'Kay Kay Industries' },
        { slug: 'gataca-performance-supplements-amazon-case-study', brandName: 'GATACA Supplements' },
        { slug: 'sai-gallery-agarbatti-brand-amazon-case-study', brandName: 'Sai Gallery' },
      ];

  const logoSrc = branding.footerLogo || branding.darkLogo || branding.headerLogo || '/images/ecomvanta-logo.png';
  const siteName = branding.siteName || 'EcomVanta';
  const phone = contactFooter.contactPhone || branding.tollFreePhone || branding.topbarPhone || '+91 878 724 9407';
  const email = contactFooter.supportEmail || branding.email || 'ecomvanta40@gmail.com';
  const address = contactFooter.officeAddress || 'New Delhi, India';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 pb-12 border-b border-slate-800">
          ...
          <div className="space-y-5">
            <Link href="/" className="inline-block bg-white p-3 rounded-2xl shadow-xl shadow-blue-500/10 transition-transform duration-300 hover:scale-102">
              <img 
                src={logoSrc} 
                alt={siteName} 
                className="h-12 sm:h-14 md:h-16 w-auto object-contain max-w-[220px] sm:max-w-[250px]"
                onError={(e) => {
                  (e.currentTarget as HTMLImageElement).src = '/images/ecomvanta-logo.png';
                }}
              />
            </Link>
            <p className="text-slate-300 text-sm leading-relaxed">
              {contactFooter.footerAboutText || branding.metaDescription || `${siteName} is your dedicated marketplace growth partner from setup to sales. We scale eCommerce businesses across Amazon, Flipkart, Meesho, Blinkit, Zepto, Myntra, and Brand D2C Websites.`}
            </p>
            ...
            <ul className="space-y-2.5 text-sm text-slate-300">
              {servicesList.map((svc: any) => (
                <li key={svc.slug}>
                  <Link 
                    href={`/${svc.slug}`} 
                    className="hover:text-[#00C2FF] hover:translate-x-1 inline-block transition-all"
                  >
                    {svc.title || svc.name}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm text-slate-300 mb-6">
              <li><Link href="/about-us" className="hover:text-[#00C2FF] hover:translate-x-1 inline-block transition-all">About {siteName}</Link></li>
            ...
            {caseStudiesList.length > 0 && (
              <>
                ...
                <ul className="space-y-1.5 text-xs text-slate-400">
                  {caseStudiesList.map((cs: any) => (
                    <li key={cs.slug}>
                      <Link href={`/${cs.slug}`} className="hover:text-white transition-colors">
                        {cs.brandName || cs.title}
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3 text-sm text-slate-300">
              ...
              <div>
                ...
                <a href={`tel:${phone}`} className="font-bold text-white hover:text-cyan-300 transition-colors">
                  {phone}
                </a>
              </div>
            </div>
        ...
        <div className="pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-slate-400 gap-4">
          <p>
            {contactFooter.copyrightText || `Copyright © ${new Date().getFullYear()} ${siteName}. All rights reserved. Powering Brands. Scaling E-commerce.`}
          </p>
